...
export async function fetchWithFallback(cid: string): Promise<any> {
  let lastError: Error | null = null;
  
  // First try Web3.Storage client
  if (web3Client) {
    try {
      return await fetchJSONFromCID(cid);
    } catch (error) {
      lastError = error as Error;
      console.warn("Web3.Storage failed, trying gateways:", error);
    }
  }
  ...
  throw new Error(`All IPFS access methods failed. Last error: ${lastError?.message}`);
}
