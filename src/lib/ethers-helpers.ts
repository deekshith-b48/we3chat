...
import { CHAT_ABI, CHAT_ADDRESS, getContractAddress } from "@/lib/contract";
...
export function getContract(signerOrProvider?: ethers.Signer | ethers.providers.Provider): ethers.Contract {
  ...
  return new ethers.Contract(CHAT_ADDRESS, CHAT_ABI, provider);
}

// Get contract instance with specific chain ID
export function getContractForChain(chainId: number, signerOrProvider?: ethers.Signer | ethers.providers.Provider): ethers.Contract {
  const provider = signerOrProvider || getProvider();
  const address = getContractAddress(chainId);
  return new ethers.Contract(address, CHAT_ABI, provider);
}
...
export async function getNetwork(): Promise<ethers.providers.Network> {
  const provider = getProvider();
...
export async function waitForTransaction(
  txHash: string, 
  confirmations = 1,
  onUpdate?: (receipt: ethers.providers.TransactionReceipt | null) => void
...
export async function retryTransaction(
  ...
  const estimatedGas = await estimateGas(contract, methodName, args);
  const gasLimit = estimatedGas.mul(Math.floor(gasMultiplier * 100)).div(100);
  
  return contract[methodName](...args, { gasLimit });
}
...
export async function checkNetworkAndConnection(): Promise<{
  ...
  try {
    ...
    const isCorrectNetwork = network.chainId === 80002; // Polygon Amoy
    
    return {
      isConnected,
      isCorrectNetwork,
      currentNetwork: network,
      currentAccount: accounts[0] || undefined
    };
  } catch (error) {
    return {
      isConnected: false,
