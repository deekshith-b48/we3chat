...
import { getSocket, type Message, type User } from '@/lib/api';
...
export function useRealTimeMessaging() {
  ...
  const handleNewMessage = useCallback((messageData: Message & { tempId?: string }) => {
    ...
    if (friendAddress) {
      ...
      const storeMessage = {
        ...
        receiver: message.sender.id === user?.id ? friendAddress : user?.address || '',
  ...
  useEffect(() => {
    ...
    const conversationId = `direct_${[user?.address, selectedFriend].sort().join('_')}`;
  ...
  }, [socket, selectedFriend, user?.address]);
