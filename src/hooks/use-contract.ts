...
export function useCreateAccount() {
  ...
  const createAccount = async (username: string): Promise<boolean> => {
    ...
    try {
      ...
      // Estimate gas
      const gasEstimate = await (contract as any).estimateGas.registerUser(username, username, '', publicKeyBytes32);
      const gasLimit = gasEstimate.mul(120).div(100); // Add 20% buffer
      
      // Send transaction
      const tx = await (contract as any).registerUser(username, username, '', publicKeyBytes32, { gasLimit });
