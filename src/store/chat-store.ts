...
export interface TransactionStatus {
  ...
  gasUsed?: string;
  errorMessage?: string;
  type: 'message' | 'friend' | 'account' | 'key';
  relatedMessageId?: string;
  relatedFriendAddress?: string;
}
...
interface ChatState {
  ...
  unreadCounts: Record<string, number>; // friendAddress -> count
  ...
  updateUnreadCount: (friendAddress: string, count: number) => void;
  markAsRead: (friendAddress: string) => void;
...
export const useChatStore = create<ChatState>()(
  persist(
    (set, get) => ({
      ...
      updateTransaction: (hash, updates) => set((state) => ({
        transactions: {
          ...
          [hash]: state.transactions[hash] ? 
            ...
            { hash, status: 'pending', timestamp: Date.now(), type: 'message', ...updates }
        }
      })),
      ...
      updateUnreadCount: (friendAddress, count) => set((state) => ({
        unreadCounts: {
          ...state.unreadCounts,
          [friendAddress]: count
        }
      ...
      markAsRead: (friendAddress) => set((state) => ({
        unreadCounts: {
          ...state.unreadCounts,
          [friendAddress]: 0
        }
